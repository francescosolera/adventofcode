...
def day3(input_file_path):
    ...
    return bin2dec(gamma(data, 1)) * bin2dec(gamma(data, 0)), f2(data, 0, 1) * f2(data, 0, 0)


def day4(input_file_path):
    """DAY 4 - ? LINES - https://adventofcode.com/2021/day/4
    """
...
def day5(input_file_path):
    ...
    return np.sum(F[:, :, 1] > 1), np.sum(np.sum(F, axis=-1) > 1)


def day6(input_file_path):
    """DAY 6 - 5 LINES - https://adventofcode.com/2021/day/6
    """
    r1, data = 0, [np.sum(np.array(list(map(int, open(input_file_path, 'r').read().split(",")))) == i).astype('int64') for i in range(9)]
    for day in range(256):
        data = data[1:7] + [data[7] + data[0]] + [data[8]] + [data[0]]
        if day < 80: r1 = sum(data)
    return r1, sum(data)
...
def day7(input_file_path):
    ...
    return res(data, fun=lambda x: x), res(data, fun=lambda x: x * (x + 1) // 2)
